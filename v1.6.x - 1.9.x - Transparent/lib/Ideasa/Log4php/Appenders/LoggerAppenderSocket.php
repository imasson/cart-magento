 ...
 */ 
class Ideasa_Log4php_Appenders_LoggerAppenderSocket extends Ideasa_Log4php_LoggerAppender {
	...
	public function activateOptions() {
		if(!$this->dry) {
			...
			if ($this->sp === false) {
				throw new Ideasa_Log4php_LoggerException("Could not open socket to ".$this->getRemoteHost().":".$this->getPort().": $errstr ($errno)");
			}
		}
		if($this->getUseXml()) {
			$this->xmlLayout = Ideasa_Log4php_LoggerReflectionUtils::createObject('Ideasa_Log4php_Layouts_LoggerLayoutXml');
	...
	public function setLocationInfo($flag) {
		$this->locationInfo = Ideasa_Log4php_Helpers_LoggerOptionConverter::toBoolean($flag, $this->getLocationInfo());
	} 
	...
	public function setLog4jNamespace($flag) {
		$this->log4jNamespace = Ideasa_Log4php_Helpers_LoggerOptionConverter::toBoolean($flag, $this->getLog4jNamespace());
	} 
	...
	public function setPort($port) {
		$port = Ideasa_Log4php_Helpers_LoggerOptionConverter::toInt($port, 0);
	...
	public function setTimeout($timeout) {
		$this->timeout = Ideasa_Log4php_Helpers_LoggerOptionConverter::toInt($timeout, $this->getTimeout());
	}
	...
	public function setUseXml($flag) {
		$this->useXml = Ideasa_Log4php_Helpers_LoggerOptionConverter::toBoolean($flag, $this->getUseXml());
	} 

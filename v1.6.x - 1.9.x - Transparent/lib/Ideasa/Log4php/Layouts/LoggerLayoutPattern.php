 ...
 *            See {@link Ideasa_Log4php_LoggerMDC} class for more details. 
 ...
 * @since 0.3 
 */
class Ideasa_Log4php_Layouts_LoggerLayoutPattern extends Ideasa_Log4php_LoggerLayout {
	...
	public function setConversionPattern($conversionPattern) {
		...
		$patternParser = new Ideasa_Log4php_Helpers_LoggerPatternParser($this->pattern);
	 ...
	 * 
	 * This method is mainly used for the prepared statements of {@see Ideasa_Log4php_Appenders_LoggerAppenderPDO}.
	 * 
	 ...
	 */
	public function formatToArray(Ideasa_Log4php_LoggerLoggingEvent $event) {
		$results = array();
		$c = $this->head;
		while ($c !== null) {
			if ( ! $c instanceOf Ideasa_Log4php_Helpers_LoggerLiteralPatternConverter) {
				$sbuf = null;
				$c->format($sbuf, $event);
				$results[] = $sbuf;
			}

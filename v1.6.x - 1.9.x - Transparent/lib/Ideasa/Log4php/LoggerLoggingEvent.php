 ...
 * @package log4php
 */
class Ideasa_Log4php_LoggerLoggingEvent {
	...
	*/
	...
	/** 
	* @var Ideasa_Log4php_LoggerLocationInfo Location information for the caller. 
	*/
	private $locationInfo = null;
	
	/**
	 * @var Ideasa_Log4php_LoggerThrowableInformation log4php internal representation of throwable
	 */
	...
	* Ideasa_Log4php_LoggerLoggingEvent are filled when actually needed.
	*
	* @param string $fqcn name of the caller class.
	* @param mixed $logger The {@link Logger} category of this event or the logger name.
	* @param Ideasa_Log4php_LoggerLevel $priority The level of this event.
	...
	*/
	public function __construct($fqcn, $logger, $priority, $message, $timeStamp = null, Exception $throwable = null) {
		$this->fqcn = $fqcn;
		if($logger instanceof Ideasa_Log4php_Logger) {
			$this->logger = $logger;
		...
		if ($throwable !== null && $throwable instanceof Exception) {
			$this->throwableInfo = new Ideasa_Log4php_LoggerThrowableInformation($throwable);
		}
	}
	 ...
	 * @return Ideasa_Log4php_LoggerLocationInfo
	 */
	public function getLocationInformation() {
		if($this->locationInfo === null) {
			...
			if(function_exists('debug_backtrace')) {
				...
				while($hop !== null) {
					if(isset($hop['class'])) {
						...
						if(!empty($className) and ($className == 'ideasa_log4php_logger' or $className == 'ideasa_log4php_loggercategory' or 
							strtolower(get_parent_class($className)) == 'ideasa_log4php_logger' or
							strtolower(get_parent_class($className)) == 'ideasa_log4php_loggercategory')) {
			...
			$this->locationInfo = new Ideasa_Log4php_LoggerLocationInfo($locationInfo, $this->fqcn);
		}
	...
	public function getMDC($key) {
		return Ideasa_Log4php_LoggerMDC::get($key);
	}
	...
	public function getTime() {
		...
		$eventStartTime = (float)Ideasa_Log4php_LoggerLoggingEvent::getStartTime();
	...
	/**
	 * @return mixed Ideasa_Log4php_LoggerThrowableInformation
	 */
...
Ideasa_Log4php_LoggerLoggingEvent::getStartTime();

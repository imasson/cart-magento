 ...
 * @since 0.4 
 */
class Ideasa_Log4php_Configurators_LoggerConfiguratorXml implements Ideasa_Log4php_LoggerConfigurator {
	...
	<log4php:configuration threshold="all">
		<appender name="A1" class="Ideasa_Log4php_Appenders_LoggerAppenderEcho">
			<layout class="Ideasa_Log4php_Layouts_LoggerLayoutSimple" />
		</appender>
		<root>
			<level value="debug" />
			<appender_ref ref="A1" />
	...
	/**
	 * @var Ideasa_Log4php_LoggerHierarchy
	 */
	private $repository;
	...
	/**
	 * @var Ideasa_Log4php_Logger parsed Logger  
	 */
	private $logger;
	
	/**
	 * @var Ideasa_Log4php_LoggerAppender parsed Ideasa_Log4php_LoggerAppender 
	 */
	private $appender;
	
	/**
	 * @var Ideasa_Log4php_LoggerFilter parsed Ideasa_Log4php_LoggerFilter 
	 */
	private $filter;
	
	/**
	 * @var Ideasa_Log4php_LoggerLayout parsed Ideasa_Log4php_LoggerLayout 
	 */
	private $layout;
	 ...
	 *
	 * @param string $url
	 * @param Ideasa_Log4php_LoggerHierarchy $repository
	 */
	private function doConfigure($url = '', Ideasa_Log4php_LoggerHierarchy $repository)
	{
		$xmlData = '';
		if (!empty($url))
	 ...
	 * @param Ideasa_Log4php_LoggerHierarchy $repository
	 */
	private function doConfigureByString($xmlData, Ideasa_Log4php_LoggerHierarchy $repository)
	{
	 ...
	 * @param Ideasa_Log4php_LoggerHierarchy $repository
	 */
	private function doConfigureDefault(Ideasa_Log4php_LoggerHierarchy $repository)
	{
	 ...
	 * @param string $xmlData
	 */
	private function parse($xmlData, Ideasa_Log4php_LoggerHierarchy $repository)
	{
			...
			case self::XMLNS.':CONFIGURATION':
				...
				if (isset($attribs['THRESHOLD'])) {
					...
					$this->repository->setThreshold(
						Ideasa_Log4php_Helpers_LoggerOptionConverter::toLevel(
			...
			case self::XMLNS.':APPENDER':
				if ($this->appender !== null) {
					if ($this->appender->requiresLayout() and $this->appender->getLayout() === null) {
						...
						$this->appender->setLayout(Ideasa_Log4php_LoggerReflectionUtils::createObject('Ideasa_Log4php_Layouts_LoggerLayoutSimple'));
					}					
